...
- 页面采用统一四按钮模型：`写入日志批次`、`随机上报日志`、`发现并上报`、`刷新快照`。
...
### 场景 6：随机上报按钮每次只上报 1 条日志

- 假如 demo 页面已在模拟器中打开，且 CLI 网关可访问
- 当我点击“随机上报日志”按钮
- 那么页面应调用 `POST /v2/logs:ingest`
- 且每次请求仅上报 1 条随机日志记录
- 并更新页面内的 ingest 结果状态
### 场景 7：核心代码不重复
...
### 场景 8：日志页客户端路由与自动上报身份一致

- 假如 H5 日志页按 `platform/appId/sessionId/deviceId` 四元组路由
- 当 Demo 自动执行 discover 后触发 `POST /v2/logs:ingest`
- 那么上报日志应使用 `sim-session-alpha` 与 `sim-device-alpha`
- 并可直接在对应客户端日志页看到实时记录
